...
| 기본 후보 | Gaussian, Log-normal, Gamma, Weibull, Log-logistic, Student-t, GMM(K=2) |
| GMM | **기본 포함**, K=2 (`--no-include-gmm`로 제외) |
| Ablation | **Local AIC vs Global mean AIC** (기본 비가중, `--weight-global-aic` 옵션) |
...
- **Global mean AIC** (코호트 \(U\), 기본):

\[
\overline{\mathrm{AIC}}_{f,m}
= \frac{1}{|U_{f,m}|}
\sum_{u \in U_{f,m}} \mathrm{AIC}_{u,f,m},
\qquad
\hat m^{\mathrm{global}}_{f}
= \arg\min_m \overline{\mathrm{AIC}}_{f,m}
\]

\(U_{f,m}\)는 해당 feature/model fit이 유효한 유저 집합.

- **Global weighted AIC** (`--weight-global-aic`):
...
코드 내부 키와 일부 CSV 컬럼명은 legacy/API 호환 때문에 `global_weighted_aic`,
`best_weighted_mean_aic`를 유지하지만, 기본값은 **비가중 mean**이다.
...
### Local vs Global — GMM 제외 (`results/evaluation_aic_selection/`, `--no-include-gmm`)

| Policy | ROC-AUC | FAR | FRR | EER |
|--------|---------|-----|-----|-----|
| **Local AIC** | **0.843** | 0.190 | 0.341 | 0.183 |
| Global mean AIC | 0.833 | 0.196 | 0.347 | 0.190 |

- Family agreement: **46.2%** (244 / 528)  
- Local이 소폭 우위.

### Local vs Global — GMM 포함, 비가중 mean (`results/evaluation_aic_selection_gmm_not_weighted/`)

| Policy | ROC-AUC | FAR | FRR | EER |
|--------|---------|-----|-----|-----|
| **Local AIC + GMM** | **0.881** | 0.161 | 0.256 | 0.143 |
| Global mean AIC + GMM | 0.879 | 0.157 | 0.324 | 0.153 |

- Family agreement: **54.7%** (289 / 528)  
- AUC는 거의 동등(local 소폭↑); FRR/EER은 local이 더 낮음.  
- Global(+GMM)은 코호트 평균 AIC에서 GMM 우세 → family가 GMM으로 쏠림.

### GMM 제외 참고 (local_aic만)

| Setting | ROC-AUC | FAR | FRR | EER |
|---------|---------|-----|-----|-----|
| −GMM (`--no-include-gmm`) | 0.843 | 0.190 | 0.341 | 0.183 |
| **기본 +GMM K=2** | **0.881** | 0.161 | 0.256 | 0.143 |
...
| 경로 | 내용 |
|------|------|
| `results/evaluation_aic_selection_gmm_not_weighted/` | local vs global mean (+GMM, 기본 비가중) + ROC 그림 |
| `results/evaluation_aic_selection_gmm/` | local vs global weighted (+GMM, `--weight-global-aic`) + ROC 그림 |
...
| `results/main_kmt/` | 분포 집계 (+GMM, train-only, 기본 비가중) |
...
- [ ] (선택) `--weight-global-aic`, `--no-include-gmm` 산출을 같은 표에 병기

---

## 6. 한 줄 요약

> Feature는 **all(6)** 고정. Auth 기본은 Local AIC + GMM, global 비교 기본은 Global mean AIC(비가중).
> +GMM: local/global AUC는 거의 동등 (0.881 vs 0.879)이나 local의 FRR/EER이 낮음.
